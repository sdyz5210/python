...
'''
	主要完成对SupportingObservations文件的翻译
'''
...
def writeExcel(rows):
	...
	table = workbook.add_sheet("SupportingObservations")
	for i in range(len(rows)):
		table.write(i,0,rows[i][0])
		table.write(i,1,rows[i][1])
		table.write(i,2,rows[i][2])
	workbook.save('SupportingObservations-translate.xls')
...
if __name__ == '__main__':
	readExcel("SupportingObservations.xlsx",True)